import WordieGuess from "./WordieGuess";
...
export default class Wordie {
  ...
  public constructor() {
    ...
    // DEBUG: Log correct word to console
  ...
  public makeGuess(guess: string): void {
      ...
      if (this.words.find(word) => word.toUpperCase() === guess.toUpperCase() === undefined) return;

      this.guesses.push(new WordieGuess)
    }
  }

  public getGuessesLeft(): number {
    return 0;
  }
  ...
  /**
   * Builds a DIV element containing all of the guesses made so far.
   * The WordieGuess class handles the collection and coloring of the letters.
   * @returns a string representation of the created div
   */
